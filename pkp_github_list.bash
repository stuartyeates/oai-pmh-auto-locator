...
LOGBASE=${DIR}/list_pkp_ojs_list_${TIMESTAMP}
...
wget --output-document=- -q https://raw.githubusercontent.com/pkp/ojsstats/master/Playing%20with%20asyncio.ipynb | \
	...
	parallel --jobs 20 'wget --no-check-certificate -O  - -q  {}?verb=Identify  --tries=2 --timeout=20 | grep "http://www.openarchives.org/OAI/2.0/" > /dev/null ; if (( 0 == $? )) ; then echo {} ;  else echo  ; fi' | \
